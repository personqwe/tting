...
Future<List<Consumption>> consumptionData(int id, String year, String month) async {
  final url = "##############/search/payment";
  ...
  if (res.statusCode == 200) {
    ...
    if (result is List) {
      print(List<Consumption>.from(result.map((item) => Consumption.fromJson(item))));
      return List<Consumption>.from(result.map((item) => Consumption.fromJson(item)));
    } else {
...
Future<List<Consumption>> consumptionListSet(
  ...
  try {
    List<Consumption> list = await consumptionData(userId, yearString, monthString);
    return list;
  } catch (error) {
    print('찾을 수 없음');
    return [];
